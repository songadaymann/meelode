...
var frameBitmap; // Apple II frame overlay
...
{
	var coverPageImages = [
		{ src: "image/introscreen.png"+noCache,  id: "cover" },
		{ src: "image/frame.png"+noCache,  id: "frame" },
	...
	function handleCoverPageFileLoad(e)
	{
		...
		case "cover":
			...
			break;
		case "frame":
			frameBitmap = new createjs.Bitmap(e.result);
			break;
		case "runner":
	...
	function addCover2Screen(image)
	{
		...
		// Position intro image using offset values from lodeRunner.main.js
		coverBitmap.setTransform(INTRO_LEFT_PCT, INTRO_TOP_PCT, INTRO_SCALE * tileScale, INTRO_SCALE * tileScale);

		addTitleBackground(coverBitmap.getBounds().width*tileScale|0 + INTRO_LEFT_PCT*2,
		                   coverBitmap.getBounds().height*tileScale|0 + INTRO_TOP_PCT*2);
	...
	function addTitleBackground(width, height)
	{
		...
		titleBackground.graphics.beginFill("#6466f1").drawRect(0, 0, canvas.width, canvas.height);
...
function preloadResource() 
{
	...
	function clearLoadingInfo()
	{
		...
		// Skip the rainbow background - keep intro image as-is
		// Skip signet and remake overlays - they covered the custom intro

		// Add "CLICK TO PLAY" text
		var clickText = new createjs.Text("CLICK TO PLAY", "bold 32px 'Press Start 2P', 'Courier New', monospace", "#FFFFFF");
		clickText.textAlign = "center";
		clickText.x = canvas.width / 2;
		clickText.y = canvas.height - 80;
		mainStage.addChild(clickText);

		// Add blinking effect
		function blink() {
			createjs.Tween.get(clickText, {loop: true})
				.to({alpha: 0.3}, 500)
				.to({alpha: 1}, 500);
		}
		blink();
		...
		mainStage.update();
		preloadComplet(); // Call directly since we removed the tween callback
	}
