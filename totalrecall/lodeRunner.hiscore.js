...
function showScoreTable(_playData, _curScoreInfo, _callbackFun, _waitTime)
{
	...
	function inputHiScoreName(winner)
	{
		...
		function inputFinish(async)
		{
			...
			// Submit to mann.cool global leaderboard
			if (window.meelodeLeaderboard && nameString && nameString.length >= 2) {
				var scoreInfo = hiScoreInfo[recordId];
				window.meelodeLeaderboard.submit(
					nameString,
					scoreInfo.s,
					scoreInfo.l,
					_playData,
					...
					function(result) {
						if (result.success) {
							console.log('Score submitted! Global rank:', result.rank);
						} else {
							console.log('Score submit failed:', result.error);
						}
					}
				);
			}
