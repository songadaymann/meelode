...
// Frame dimensions and screen area within frame (percentages)
var FRAME_WIDTH = 2784;
var FRAME_HEIGHT = 1536;
var FRAME_ASPECT = FRAME_WIDTH / FRAME_HEIGHT; // 1.8125

// Screen area within frame (as percentages of frame size)
// Tuned values for the Apple II frame
var SCREEN_LEFT_PCT = 0.099;
var SCREEN_TOP_PCT = -0.012;
var SCREEN_WIDTH_PCT = 0.804;
var SCREEN_HEIGHT_PCT = 1.000;

// Position adjustment mode - set to true to enable tuning with arrow keys
var positionAdjustMode = false;

// Intro image position (pixels offset)
var INTRO_LEFT_PCT = 50;
var INTRO_TOP_PCT = 50;
var INTRO_SCALE = 1.0;
var introAdjustMode = false; // Disabled - values are set

function setupPositionAdjustment() {
	window.addEventListener('keydown', function(e) {
		// Handle intro image adjustment
		if (introAdjustMode && gameState === GAME_WAITING) {
			var step = 5; // pixels
			var scaleStep = 0.02;
			var changed = false;
			...
			switch(e.key) {
				...
				case '=':
				case '+':
					INTRO_SCALE += scaleStep;
					changed = true;
					break;
				case '-':
				case '_':
					INTRO_SCALE -= scaleStep;
					if (INTRO_SCALE < 0.1) INTRO_SCALE = 0.1;
					changed = true;
					break;
				case 'p':
				case 'P':
					console.log('=== INTRO POSITION VALUES ===');
					console.log('var INTRO_LEFT_PCT = ' + INTRO_LEFT_PCT.toFixed(1) + ';');
					console.log('var INTRO_TOP_PCT = ' + INTRO_TOP_PCT.toFixed(1) + ';');
					console.log('var INTRO_SCALE = ' + INTRO_SCALE.toFixed(2) + ';');
					console.log('=============================');
					alert('Intro position values:\n\n' +
						'LEFT: ' + INTRO_LEFT_PCT.toFixed(1) + '\n' +
						'TOP: ' + INTRO_TOP_PCT.toFixed(1) + '\n' +
						'SCALE: ' + INTRO_SCALE.toFixed(2));
					break;
			}
			...
			if (changed) {
				e.preventDefault();
				updateIntroPosition();
				console.log('INTRO - LEFT:', INTRO_LEFT_PCT.toFixed(1),
							'TOP:', INTRO_TOP_PCT.toFixed(1),
							'SCALE:', INTRO_SCALE.toFixed(2));
			}
			return;
		...
		// Handle game canvas adjustment (disabled by default)
		...
		switch(e.key) {
			...
			case '=':
			case '+':
				SCREEN_WIDTH_PCT += step;
				SCREEN_HEIGHT_PCT += step;
				changed = true;
				break;
			case '-':
			case '_':
	...
	console.log('=== INTRO ADJUSTMENT MODE ===');
	console.log('Arrow keys: move intro image');
	console.log('+/-: scale intro image');
	console.log('P: print values to copy');
	console.log('=============================');
}

// Update intro image position
function updateIntroPosition() {
	if (coverBitmap) {
		coverBitmap.setTransform(INTRO_LEFT_PCT, INTRO_TOP_PCT, INTRO_SCALE * tileScale, INTRO_SCALE * tileScale);
		mainStage.update();
	}
}
...
function canvasReSize()
{
	...
	// Keep internal canvas at base resolution (for game logic)
	tileScale = 1;
	canvasX = BASE_SCREEN_X; // 1120
	canvasY = BASE_SCREEN_Y; // 768

	canvas.width = canvasX;
	canvas.height = canvasY;

	// Calculate frame size to fit viewport
	...
	if (viewportAspect > FRAME_ASPECT) {
		// Viewport is wider than frame - fit to height
		frameH = viewportH;
		frameW = frameH * FRAME_ASPECT;
	} else {
		// Viewport is taller than frame - fit to width
		frameW = viewportW;
		frameH = frameW / FRAME_ASPECT;
	}

	// Set frame wrapper size
	frameWrapper.style.width = frameW + 'px';
	frameWrapper.style.height = frameH + 'px';

	// Calculate screen area within frame (in pixels)
	var screenLeft = frameW * SCREEN_LEFT_PCT;
	var screenTop = frameH * SCREEN_TOP_PCT;
	var screenW = frameW * SCREEN_WIDTH_PCT;
	var screenH = frameH * SCREEN_HEIGHT_PCT;

	// Position and scale canvas to fit within screen area
	// Maintain game aspect ratio within the screen area
	var gameAspect = canvasX / canvasY;
	var screenAspect = screenW / screenH;

	var displayW, displayH;
	if (gameAspect > screenAspect) {
		// Game is wider - fit to screen width
		displayW = screenW;
		displayH = displayW / gameAspect;
	} else {
		// Game is taller - fit to screen height
		displayH = screenH;
		displayW = displayH * gameAspect;
	}

	// Always center both horizontally and vertically within screen area
	var displayLeft = screenLeft + (screenW - displayW) / 2;
	var displayTop = screenTop + (screenH - displayH) / 2;
	...
	// Update tile scale variables for rendering
	tileW = BASE_TILE_X;
	tileH = BASE_TILE_Y;
	...
	debug("Responsive: frameW=" + frameW + " frameH=" + frameH + " displayW=" + displayW + " displayH=" + displayH);
}
...
function showCoverPage()
{
	...
	// Removed signetBitmap and remakeBitmap overlays
	mainStage.update();
	waitIdleDemo(3000);
}
...
{
	// Auto-demo disabled - just wait for user interaction
	anyKeyStopDemo();
}
...
{
	// Always start on Classic Lode Runner level 1
...
function startGame(noCycle)
{
	...
	case PLAY_CLASSIC:
		getClassicInfo();
		curLevel = 1; // Always start on level 1
	...
	case PLAY_MODERN:
		...
		break;
	case PLAY_GENERATED:
		// Generate a new solvable level using Markov chain + A* verification
		levelMap = generateSolvableLevel(6, 3, 20);
		if(!levelMap) {
			console.error('Failed to generate level, falling back to classic');
...
function closingScreen(r)
{
	...
	if(r > 0) {
		...
		if(playMode == PLAY_TEST) {
			levelMap = getTestLevelMap();
		} else if(playMode == PLAY_GENERATED) {
			levelMap = generateSolvableLevel(6, 3, 20);
		} else {
			levelMap = levelData[curLevel-1];
		}
...
function mainTick(event)
{
	...
	case GAME_FINISH: 
		...
		case PLAY_GENERATED:
			soundPlay(soundEnding);
			// Generate a new level after completing
			setTimeout(function() {gameState = GAME_NEW_LEVEL; }, 1000);
			gameState = GAME_WAITING;
			break;
		case PLAY_TEST:
			soundPlay(soundEnding);
